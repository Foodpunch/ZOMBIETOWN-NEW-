...
public class OVRMainMenu : MonoBehaviour
{
	...
	// Hugo - Control Options menu
	private bool  ShowControlOptions;
	private bool  OldControlOptionsHit;

	// Hugo - Camera blinking on rotation
	public float BlinkInSpeed = 10.0f;
	public float BlinkOutSpeed = 20.0f;
	public float MaxAlphaBlinkValue = 0.90f;
	//public float MaxAlphaBlinkIncrement = 0.05f;
	//private string strMaxAlphaBlinkValue = "";
	private float AlphaBlinkValue = 0.0f;
	
	// Hugo
	private string strControlMode = "";
	private string strBlinkOnRotation = "";
	private string strTurnAngle = "";
	private string strProgressiveTurning = "";
	private string strTurnSpeed = "";
	private string strRotationScaleMultiplier = "";
	...
	void Start()
	{
		...
		// Hugo
		UpdateFunctions += UpdateControlOptions;
	}
	...
	// OnGUI
 	...
 	{	
		...
		// Hugo - Blink on rotation
		if (PlayerController.BlinkOnRotation) {
			// Blink in
			if (PlayerController.BlinkState == OVRPlayerController.BLINK_IN) {
				if(AlphaBlinkValue > 0.0f) {
					AlphaBlinkValue -= Time.deltaTime * BlinkInSpeed;
					if(AlphaBlinkValue < 0.0f) {
						AlphaBlinkValue = 0.0f;	
					} else {
						GUI.color = new Color(0, 0, 0, AlphaBlinkValue);
						GUI.DrawTexture( new Rect(0, 0, Screen.width, Screen.height ), FadeInTexture ); 
					}
				} else {
					PlayerController.BlinkState = OVRPlayerController.BLINK_NONE;
				}
			}

			// Blink out
			if (PlayerController.BlinkState == OVRPlayerController.BLINK_OUT) {
				AlphaBlinkValue += Time.deltaTime * BlinkOutSpeed;
				if(AlphaBlinkValue > MaxAlphaBlinkValue) {
					AlphaBlinkValue = MaxAlphaBlinkValue;	
				}
				GUI.color = new Color(0, 0, 0, AlphaBlinkValue);
				GUI.DrawTexture( new Rect(0, 0, Screen.width, Screen.height ), FadeInTexture ); 
			}
		...
		if(GUIRenderObject != null)
		{
			// Hugo - added ShowControlOptions
			if (ScenesVisible || ShowVRVars || Crosshair.IsCrosshairVisible() || ShowControlOptions ||
	...
	void GUIShowVRVariables()
	{
		...
		if ((OldSpaceHit == false) && (SpaceHit == true))
		{
			if(ShowVRVars == true) {
				...
				ShowVRVars = true;
				// Hugo - Make sure we don't display both
				ShowControlOptions = false;
			}
		}
	...
	void UpdateControlOptions() {
		/*if(Input.GetKeyDown(KeyCode.PageDown) && MaxAlphaBlinkValue > 0.0f) {	
			MaxAlphaBlinkValue -= MaxAlphaBlinkIncrement;
		} else if (Input.GetKeyDown(KeyCode.PageUp) && MaxAlphaBlinkValue < 1.0f) {
			MaxAlphaBlinkValue += MaxAlphaBlinkIncrement;
		}*/
		
		if(ShowControlOptions) {
			strControlMode = "Ctrl Mode: " + Enum.GetName(typeof(OVRPlayerController.ControlMode), PlayerController.CurrentControlMode);
			strBlinkOnRotation = "Blink turning: " + (PlayerController.BlinkOnRotation ? "Yes" : "No");
			//strMaxAlphaBlinkValue = System.String.Format ("Blink level: {0:F2}", MaxAlphaBlinkValue);
			strTurnAngle = System.String.Format ("Turn Angle: {0}", PlayerController.TurnAngle);
			strProgressiveTurning = "Smooth turns: " + (PlayerController.SmoothTurn ? "Yes" : "No");
			strTurnSpeed = System.String.Format ("Turn speed: {0}", PlayerController.TurnSpeed);
			if (PlayerController.CurrentControlMode == (int)OVRPlayerController.ControlMode.HeadStick) {
				strRotationScaleMultiplier = System.String.Format ("Sensitivity : {0}", PlayerController.HeadStickRotationScaleMultiplier);
			} else {
				strRotationScaleMultiplier = System.String.Format ("Sensitivity : {0}", PlayerController.GamepadRotationScaleMultiplier);
			}
		}
	}

	void GUIShowControlOptions()
	{
		bool ControlOptionsHit = OVRGamepadController.GPC_GetButton((int)OVRGamepadController.Button.Back);
		if ((OldControlOptionsHit == false) && (ControlOptionsHit == true))
		{
			if(ShowControlOptions) {
				ShowControlOptions = false;
			} else { 
				ShowControlOptions = true;
				// Make sure we don't display both
				ShowVRVars = false;
			}
		}
		
		OldControlOptionsHit = ControlOptionsHit;
		...
		if (ShowControlOptions) {
			...
			if(PlayerController != null) {
				// Control mode
				GuiHelper.StereoBox (VRVarsSX, y += StepY, VRVarsWidthX, VRVarsWidthY, 
				                     ref strControlMode, Color.white);
				// Alpha Blink Value
				/*GuiHelper.StereoBox (VRVarsSX, y += StepY, VRVarsWidthX, VRVarsWidthY, 
				                     ref strMaxAlphaBlinkValue, Color.white);*/

				if (PlayerController.CurrentControlMode >= (int)OVRPlayerController.ControlMode.HeadStick) {
					if (PlayerController.CurrentControlMode != (int)OVRPlayerController.ControlMode.TurnButton) {
						// Turn angle
						GuiHelper.StereoBox (VRVarsSX, y += StepY, VRVarsWidthX, VRVarsWidthY, 
						                     ref strTurnAngle, Color.white);
					}
				}
				if (PlayerController.CurrentControlMode >= (int)OVRPlayerController.ControlMode.FixedAngle) {
					// Smooth turns
					GuiHelper.StereoBox (VRVarsSX, y += StepY, VRVarsWidthX, VRVarsWidthY, 
					                     ref strProgressiveTurning, Color.white);
					// Turn speed
					if (PlayerController.SmoothTurn) {
						GuiHelper.StereoBox (VRVarsSX, y += StepY, VRVarsWidthX, VRVarsWidthY, 
						                     ref strTurnSpeed, Color.white);
					}
				} else {
					// Rotation scale (Sensitivity)
					GuiHelper.StereoBox (VRVarsSX, y += StepY, VRVarsWidthX, VRVarsWidthY, 
					                     ref strRotationScaleMultiplier, Color.white);
				}
				if (PlayerController.CurrentControlMode < (int)OVRPlayerController.ControlMode.FixedAngle || PlayerController.SmoothTurn) {
					// Camera blinks on rotation?
					GuiHelper.StereoBox (VRVarsSX, y += StepY, VRVarsWidthX, VRVarsWidthY, 
					                     ref strBlinkOnRotation, Color.white);
				}
			}
		}
	}
